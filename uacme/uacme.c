...
bool eab_parse(acme_t *, char *);
char *find_header(const char*, const char *);
#endif
...
char *strcasestr(const char *haystack, const char *needle)
{
    ...
    if (!_haystack || !needle)
		fprintf(stderr, "strcasestr: strdup failed");
    else {
        char *p;
        for (p = _haystack; *p; p++)
            *p = tolower(*p);
...
#else
	warnx("not searching for alternate certificates on plan9, falling back to default");
#endif
